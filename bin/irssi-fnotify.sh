...
(
	...
	read network from msg; do
		...
		if [ ${network:0:1} = "#" ]; then
			...
			$NOTIFY --icon=/home/lkurusa/sw/irssi-logo.png --app-name=irssi -t 300000 -- "${network}(${from})" "${fmsg}";
		else
			$NOTIFY --icon=/home/lkurusa/sw/irssi-logo.png --app-name=irssi -t 300000 -- "pvt(${network})" "${from} ${xmsg}";
		fi
		$BEEP
	done
) &
