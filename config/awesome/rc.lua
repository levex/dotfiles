...
local scrlocker    = "/home/lkurusa/bin/lock.sh"

awful.util.terminal = terminal
awful.util.tagnames = { "1 - local", "2 - remote", "3 - www", "4 - files", "5 - latex", "6 - misc" }
...
-- {{{ Naughty notifications should be sane - LK
...
table.insert(naughty.dbus.config.mapping, {{appname = "irssi"}, naughty.config.presets.irssi})
table.insert(naughty.dbus.config.mapping, {{appname = "newsboat"}, naughty.config.presets.irssi})

--beautiful.notification_bg = "#60a9d1"
--beautiful.notification_fg = "#011b28"
--beautiful.notification_width = 240
--beautiful.notification_height = 135
...
globalkeys = my_table.join(
    ...
    awful.key({ "Control", altkey }, "p", function() os.execute("gnome-screenshot") end,
              {description = "take a screenshot", group = "hotkeys"}),

    -- X screen locker
    awful.key({ "Control", "Shift" }, "l", function () os.execute(scrlocker) end,
              {description = "lock screen", group = "hotkeys"}),

    awful.key({ "Control", "Shift", modkey }, "l", function () os.execute('systemctl suspend') os.execute(scrlocker) end,
              {description = "lock screen", group = "hotkeys"}),

    -- Screenshot keys 
    awful.key({ }, "Print", function() awful.util.spawn_with_shell("sleep 0.5 && scrot '%Y-%m-%d_%H-%M-%S.png' -e 'mv $f ~/screenshots/'") end,
              {description = "take a fullscreen screenshot", group="screenshots"}),
    awful.key({ "Shift" }, "Print", function() awful.util.spawn_with_shell("sleep 0.5 && scrot --select '%Y-%m-%d_%H-%M-%S.png' -e 'mv $f ~/screenshots/'") end,
              {description = "take a selection screenshot", group="screenshots"}),
    awful.key({ "Control" }, "Print", function() awful.util.spawn_with_shell("sleep 0.5 && scrot --focused '%Y-%m-%d_%H-%M-%S.png' -e 'mv $f ~/screenshots/'") end,
              {description = "take a screenshot of the focused window", group="screenshots"}),
    -- awful.key({ "Shift", "Control" }, "Print", function() awful.util.spawn_with_shell("ls -la ~/screenshots | sort -k 9 -r | head -1 | awk '{ print $9 }' | xargs -I {} imgur-uploader ~/screenshots/{} 2>&1 | grep -m 1 see | awk '{ print $7 }' | xclip -selection c") end,
    awful.key({ "Shift", "Control" }, "Print", function() awful.util.spawn_with_shell("/home/lkurusa/bin/ahti-upload-screenshot") end,
              {description = "upload the last screenshot", group="screenshots"}),
    -- Hotkeys
...
clientkeys = my_table.join(
    ...
    awful.key({ modkey, "Shift"   }, "s",  function(c) c.sticky = not c.sticky end,
              {description = "toggle stickiness", group="client"}),
